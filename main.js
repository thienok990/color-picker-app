...
let colorPicker = {
    ...
    addColor: function(){
        const nameColor = document.querySelector(this.selectors.nameColor).value.trim();
    ...
    registerMainEvents: function(){
        ...
        document
          .querySelectorAll(this.selectors.copyButton).forEach(element => {
              const that = this;
              element.addEventListener('click', function(){
                    const currentColor = that.colors.find(color => color.id == element.getAttribute('data-id'));
    ...
    currentColors: function(){
        if(this.colors.length > 1){
            document.querySelector(this.selectors.totalColors).innerHTML = this.colors.length + ' Colors';
        }
        else{
            document.querySelector(this.selectors.totalColors).innerHTML = this.colors.length + ' Color';
        } 
    },
    searchColor: function(){
        const searchColor = document.querySelector(this.selectors.searchColor).value;
